...
Hide Data in Images
    ...
    def main(self,root):
        ...
        b_encode=Button(f,text='Encode',padx=10,command= lambda :self.frame1_encode(f))
        b_encode.config(font=('Helvetica',30))
        b_encode.grid(pady=10)
        b_decode=Button(f,text='Decode',padx=10,command= lambda :self.frame1_decode(f))
        b_decode.grid(pady=10)
        b_decode.config(font=('Helvetica',30))

        ascii_art=Label(f,text=self.art)
        ascii_art.config(font=('Helvetica',35,"bold italic"))

        ascii_art2=Label(f,text=self.art2)
        ascii_art2.config(font=('Helvetica',35,"bold italic"))
    ...
    def frame2_encode(self,f2):
            ...
            if not myfile:
                ...
                l3.config(font=('Helvetica',30,"bold italic"))
                l3.grid(pady=10)
                ...
                l2.grid(pady=10)
                text_area = Text(ep, width=60, height=10)
                text_area.grid(pady=10)
                encode_button = Button(ep, text='Cancel', command=lambda : hide.home(self,ep))
                encode_button.config(font=('Helvetica',30))
    ...
    def frame2_decode(self,d_f2):
        d_f3=Frame(root)
        myfile=tkinter.filedialog.askopenfilename(filetypes=([('png','*.png'),('jpeg','*.jpeg'),('jpg','*jpg'),('Al Files','*.*')]))
        if not myfile:
            ...
            show_info.config(font=('Helvetica',30))
